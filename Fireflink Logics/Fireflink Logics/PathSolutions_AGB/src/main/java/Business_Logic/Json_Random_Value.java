...
@Component("LIC16279_PJT1002_PE_NLPf52e527d-a978-43d0-8fdc-66c962d475fb")

public class Json_Random_Value implements Nlp {
	    ...
	    @ReturnType(name = "UpdatedJson", type = "java.lang.String")	
	      ...
	      public static Map<String, String> replacePlaceholdersWithRandomStrings(Map<String, String> jsonMap,Map<String, String> comMap) {
	          for (Map.Entry<String, String> mapData : jsonMap.entrySet()) {
	              String key = mapData.getKey();
	              String value = mapData.getValue();
	              String random="RAN_STR>>";
	              if(!value.startsWith(random)) {
	              value=getCellConvertData(comMap.get(value));
	              jsonMap.put(key, getConvertData(value));  
	              }else {
	            	  jsonMap.put(key, getConvertData(value));
	              }
	          }
