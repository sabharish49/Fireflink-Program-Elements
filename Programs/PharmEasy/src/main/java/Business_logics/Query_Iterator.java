...
public class Query_Iterator implements Nlp {
	...
	@InputParams({@InputParam(name = "MapData", type = "java.util.Map")})
      ...
      public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
		...
		Map<String, String> parameter = (Map<String, String>) attributes.get("MapData"); 
          WebDriver driver = nlpRequestModel.getWebDriver();

      	  try {
	        int Queries_Count = Integer.parseInt(parameter.get("Queries_Count"));
	       // String dataBase = paramMap.get("Databases");	        
	        ...
	        for (int i = 1; i <= Queries_Count; i++) {		        
	            ...
	            String queryValue = parameter.get(queryKey);	
	            List<String> resultList = storeSubstringBeforeColon(queryValue);
	            String result = extractSubstringAfterColon(queryValue);
	           ...
	           Thread.sleep(2000);
	            QueryTextField.sendKeys(result);
